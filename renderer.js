    ...
    */
    ...
    function Renderer (canvas, wgl, gridDimensions, onLoaded) {
        ...
        wgl.createProgramsFromFiles({
            ...
            // Initialize color uniforms with default values
            var wgl = this.wgl;
            var initialDrawState = wgl.createDrawState()
                .useProgram(this.compositeProgram)
                .uniform1f('u_liquidHue', 0.6)
                .uniform1f('u_liquidSaturation', 0.75)
                .uniform1f('u_liquidBrightness', 1.0);
            wgl.drawArrays(initialDrawState, wgl.POINTS, 0, 1);
    ...
    Renderer.prototype.updateLiquidColor = function (hue, saturation, brightness) {
        var wgl = this.wgl;
        
        // Update uniforms in the composite program using draw state
    ...
    // Thêm phương thức mới để cập nhật nhiều màu
    Renderer.prototype.updateMultipleColors = function (colorArray) {
        // colorArray là mảng các đối tượng {h, s, v}
        var wgl = this.wgl;
        
        // Chỉ cập nhật nếu có màu
        if (colorArray && colorArray.length > 0) {
            // Tính toán và gửi các giá trị màu đến shader
            var totalColors = Math.min(colorArray.length, 8); // Tối đa 8 màu do shader hỗ trợ 8 phân đoạn
            
            // Tính toán saturation và brightness trung bình để có độ sáng nhất quán
            var avgSat = 0;
            var avgVal = 0;
            
            for (var i = 0; i < colorArray.length; i++) {
                avgSat += colorArray[i].s;
                avgVal += colorArray[i].v;
            }
            
            avgSat /= colorArray.length;
            avgVal /= colorArray.length;
            
            // Tăng giá trị brightness để màu sắc nổi bật hơn
            avgVal = Math.min(1.0, avgVal * 1.2);
            
            // Tạo drawState với các uniform cơ bản
            var drawState = wgl.createDrawState()
                .useProgram(this.compositeProgram)
            ...
            // Truyền từng giá trị hue riêng lẻ cho mỗi màu
            if (totalColors >= 1) drawState.uniform1f('u_hue0', colorArray[0].h);
            if (totalColors >= 2) drawState.uniform1f('u_hue1', colorArray[1].h);
            if (totalColors >= 3) drawState.uniform1f('u_hue2', colorArray[2].h);
            if (totalColors >= 4) drawState.uniform1f('u_hue3', colorArray[3].h);
            if (totalColors >= 5) drawState.uniform1f('u_hue4', colorArray[4].h);
            if (totalColors >= 6) drawState.uniform1f('u_hue5', colorArray[5].h);
            if (totalColors >= 7) drawState.uniform1f('u_hue6', colorArray[6].h);
            if (totalColors >= 8) drawState.uniform1f('u_hue7', colorArray[7].h);
