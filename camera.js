...
var Camera = (function () {
    ...
    function Camera (element, orbitPoint) {
        ...
        // Thêm mảng listeners để thông báo khi camera thay đổi
    ...
    // Thêm phương thức để đăng ký sự kiện thay đổi
    Camera.prototype.addEventListener = function (eventType, callback) {
        if (eventType === 'change' && typeof callback === 'function') {
            this.changeListeners.push(callback);
        }
    };
    
    // Thêm phương thức để thông báo khi có thay đổi
    Camera.prototype.notifyChangeListeners = function() {
        for (var i = 0; i < this.changeListeners.length; i++) {
            try {
                this.changeListeners[i]();
            } catch (error) {
                console.error('Error in camera change listener:', error);
            }
        }
    };
    ...
    Camera.prototype.recomputeViewMatrix = function () {
        ...
        // Thông báo cho tất cả listeners khi view matrix thay đổi
